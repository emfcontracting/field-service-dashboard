...
// Revenue (VK) = CBRE billing rates ($64/$96 + markups)
// Cost    (EK) = actual tech wages + material purchase price
// Profit       = Revenue - Cost
// ─────────────────────────────────────────────────────────────────────────────
'use client';
...
export default function ProfitabilityTab({ workOrder, dailyHoursLog, dailyTotals }) {
  ...
  // ── Revenue (VK) Calculations — what CBRE pays us ────────────────────────
  const BILLING_RT   = 64;
  const BILLING_OT   = 96;
  const MARKUP       = 1.25;
  ...
  // ── Cost (EK) Calculations — what EMF actually pays ──────────────────────
  ...
  const materialCost  = materialBase;     // EK
  const equipmentCost = equipmentBase;    // EK
  const trailerCost   = trailerBase;      // EK
  const rentalCost    = rentalBase;       // EK
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard label="Revenue (VK)" value={fmt(totalRevenue)} sub="Billed to CBRE" color="blue" />
        <StatCard label="Cost (EK)"    value={fmt(totalCost)}    sub="What EMF pays" color="orange" />
        <StatCard
          label="Profit"
      ...
      <div className="bg-[#0d0d14] border border-[#1e1e2e] rounded-xl overflow-hidden">
        ...
        <div className="grid grid-cols-4 gap-2 py-2 px-3 bg-[#1e1e2e] text-xs uppercase tracking-wider text-slate-500 font-semibold">
          <span>Category</span>
          <span className="text-right">EK (Cost)</span>
          <span className="text-right">VK (Revenue)</span>
        ...
        <Row label={`Materials (EK: ${fmt(materialBase)} → VK: +25%)`} ek={materialBase} vk={materialRevenue} profit={materialRevenue - materialBase} />
        {equipmentBase > 0 && <Row label={`Equipment (EK: ${fmt(equipmentBase)} → VK: +25%)`} ek={equipmentBase} vk={equipmentRevenue} profit={equipmentRevenue - equipmentBase} highlight />}
        {trailerBase > 0   && <Row label={`Trailer (EK: ${fmt(trailerBase)} → VK: +25%)`}   ek={trailerBase}   vk={trailerRevenue}   profit={trailerRevenue - trailerBase} />}
        {rentalBase > 0    && <Row label={`Rental (EK: ${fmt(rentalBase)} → VK: +25%)`}     ek={rentalBase}    vk={rentalRevenue}    profit={rentalRevenue - rentalBase}   highlight />}
      ...
      <div className="text-xs text-slate-600 space-y-0.5 px-1">
        <div>• <strong className="text-slate-500">EK (Einkaufspreis)</strong> = What EMF pays: actual tech wages + purchase price of materials/equipment</div>
        <div>• <strong className="text-slate-500">VK (Verkaufspreis)</strong> = What CBRE pays: $64/h RT, $96/h OT, materials/equipment + 25% markup</div>
