...
export default function WorkOrdersFilters({
  ...
  const toggleTech = (value) => {
    setSelectedTechs(prev => prev.includes(value) ? prev.filter(v => v !== value) : [...prev, value]);
  };
  const togglePriority = (value) => {
    setSelectedPriorities(prev => prev.includes(value) ? prev.filter(v => v !== value) : [...prev, value]);
  };
  const toggleCbreStatus = (value) => {
    setSelectedCbreStatuses(prev => prev.includes(value) ? prev.filter(v => v !== value) : [...prev, value]);
  };

  // Select all functions
  const toggleAllStatuses = () => {
    setSelectedStatuses(prev => prev.length === statusOptions.length ? [] : statusOptions.map(o => o.value));
  };
  const toggleAllTechs = () => {
    const allTechIds = allUsers.map(u => u.user_id);
    setSelectedTechs(prev => prev.length === allTechIds.length ? [] : allTechIds);
  };
  const toggleAllPriorities = () => {
    setSelectedPriorities(prev => prev.length === priorityOptions.length ? [] : priorityOptions.map(o => o.value));
  };
  const toggleAllCbreStatuses = () => {
    setSelectedCbreStatuses(prev => prev.length === cbreStatusOptions.length ? [] : cbreStatusOptions.map(o => o.value));
  };
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-3 md:p-4 mb-4 md:mb-6">
      ...
      <input
        ...
        className="w-full bg-gray-700 text-white px-3 py-2.5 rounded-lg mb-3 text-sm md:text-base"
      />

      {/* Filter Dropdowns - Grid on mobile, flex on desktop */}
      <div className="grid grid-cols-2 gap-2 md:flex md:flex-wrap md:gap-3 mb-3">
        {/* Status Filter */}
        <div className="relative">
          <button
            onClick={() => setShowStatusDropdown(!showStatusDropdown)}
            className={`status-dropdown-btn w-full md:w-auto px-3 py-2 rounded-lg flex items-center justify-between md:justify-start gap-2 text-sm ${
          ...
          {showStatusDropdown && (
            <div className="status-dropdown absolute top-full left-0 mt-1 bg-gray-700 rounded-lg shadow-xl z-50 min-w-[180px] max-w-[90vw]">
              ...
              {statusOptions.map(opt => (
                <div key={opt.value} className="px-3 py-2 hover:bg-gray-600 cursor-pointer flex items-center gap-2" onClick={() => toggleStatus(opt.value)}>
                  <input type="checkbox" checked={selectedStatuses.includes(opt.value)} onChange={() => {}} className="w-4 h-4" />
                  <span className={`px-2 py-0.5 rounded text-xs font-semibold ${opt.color}`}>{opt.label}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="relative">
          <button
            ...
            className={`tech-dropdown-btn w-full md:w-auto px-3 py-2 rounded-lg flex items-center justify-between md:justify-start gap-2 text-sm ${
              selectedTechs.length > 0 ? 'bg-blue-700 text-white' : 'bg-gray-700 text-white'
            }`}
          >
            <span className="flex items-center gap-1">
              <span>👷</span>
              <span className="truncate">{getTechButtonText()}</span>
            </span>
            <span>▼</span>
          </button>
          
          {showTechDropdown && (
            <div className="tech-dropdown absolute top-full left-0 mt-1 bg-gray-700 rounded-lg shadow-xl z-50 min-w-[200px] max-w-[90vw] max-h-[300px] overflow-y-auto">
              <div className="px-3 py-2 hover:bg-gray-600 cursor-pointer border-b border-gray-600 flex items-center gap-2" onClick={toggleAllTechs}>
                <input type="checkbox" checked={selectedTechs.length === allUsers.length} onChange={() => {}} className="w-4 h-4" />
                <span className="font-semibold text-sm">Select All</span>
              </div>
              <div className="px-3 py-2 hover:bg-gray-600 cursor-pointer flex items-center gap-2 border-b border-gray-600" onClick={() => toggleTech('unassigned')}>
                <input type="checkbox" checked={selectedTechs.includes('unassigned')} onChange={() => {}} className="w-4 h-4" />
                <span className="text-yellow-400 text-sm">⚠️ Unassigned</span>
              </div>
              {techs.length > 0 && (
                <>
                  <div className="px-3 py-1 text-xs text-gray-400 bg-gray-800">── Field Techs ──</div>
                  {techs.map(tech => (
              ...
              {admins.length > 0 && (
                <>
                  <div className="px-3 py-1 text-xs text-gray-400 bg-gray-800">── Admin/Office ──</div>
                  {admins.map(admin => (
                    <div key={admin.user_id} className="px-3 py-2 hover:bg-gray-600 cursor-pointer flex items-center gap-2" onClick={() => toggleTech(admin.user_id)}>
                      <input type="checkbox" checked={selectedTechs.includes(admin.user_id)} onChange={() => {}} className="w-4 h-4" />
        ...
        <div className="relative">
          <button
            ...
            className={`priority-dropdown-btn w-full md:w-auto px-3 py-2 rounded-lg flex items-center justify-between md:justify-start gap-2 text-sm ${
              selectedPriorities.length > 0 ? 'bg-blue-700 text-white' : 'bg-gray-700 text-white'
            }`}
          >
            <span className="flex items-center gap-1">
              <span>🎯</span>
              <span className="truncate">{getPriorityButtonText()}</span>
            </span>
            <span>▼</span>
          </button>
          
          {showPriorityDropdown && (
            <div className="priority-dropdown absolute top-full left-0 mt-1 bg-gray-700 rounded-lg shadow-xl z-50 min-w-[180px] max-w-[90vw] max-h-[280px] overflow-y-auto">
              <div className="px-3 py-2 hover:bg-gray-600 cursor-pointer border-b border-gray-600 flex items-center gap-2" onClick={toggleAllPriorities}>
                <input type="checkbox" checked={selectedPriorities.length === priorityOptions.length} onChange={() => {}} className="w-4 h-4" />
                <span className="font-semibold text-sm">Select All</span>
              </div>
              {priorityOptions.map(opt => (
                <div key={opt.value} className="px-3 py-2 hover:bg-gray-600 cursor-pointer flex items-center gap-2" onClick={() => togglePriority(opt.value)}>
                  <input type="checkbox" checked={selectedPriorities.includes(opt.value)} onChange={() => {}} className="w-4 h-4" />
                  <span className={`px-2 py-0.5 rounded text-xs font-semibold ${opt.color}`}>{opt.label}</span>
                </div>
              ))}
            </div>
        ...
        <div className="relative">
          <button
            ...
            className={`cbre-dropdown-btn w-full md:w-auto px-3 py-2 rounded-lg flex items-center justify-between md:justify-start gap-2 text-sm ${
              selectedCbreStatuses.length > 0 ? 'bg-purple-700 text-white' : 'bg-gray-700 text-white'
            }`}
          >
            <span className="flex items-center gap-1">
              <span>📧</span>
              <span className="truncate">{getCbreButtonText()}</span>
            </span>
            <span>▼</span>
          </button>
          
          {showCbreDropdown && (
            <div className="cbre-dropdown absolute top-full left-0 mt-1 bg-gray-700 rounded-lg shadow-xl z-50 min-w-[180px] max-w-[90vw]">
              <div className="px-3 py-2 hover:bg-gray-600 cursor-pointer border-b border-gray-600 flex items-center gap-2" onClick={toggleAllCbreStatuses}>
                <input type="checkbox" checked={selectedCbreStatuses.length === cbreStatusOptions.length} onChange={() => {}} className="w-4 h-4" />
                <span className="font-semibold text-sm">Select All</span>
              </div>
              {cbreStatusOptions.map(opt => (
                <div key={opt.value} className="px-3 py-2 hover:bg-gray-600 cursor-pointer flex items-center gap-2" onClick={() => toggleCbreStatus(opt.value)}>
                  <input type="checkbox" checked={selectedCbreStatuses.includes(opt.value)} onChange={() => {}} className="w-4 h-4" />
                  <span className={`px-2 py-0.5 rounded text-xs font-semibold ${opt.color}`}>{opt.label}</span>
                </div>
              ))}
            </div>
      ...
      <div className="flex flex-wrap gap-2">
        <button onClick={onNewWorkOrder} className="flex-1 md:flex-none bg-green-600 active:bg-green-700 md:hover:bg-green-700 px-3 py-2.5 rounded-lg font-semibold transition text-sm">
          + New WO
        </button>
        <button onClick={onImport} className="flex-1 md:flex-none bg-blue-600 active:bg-blue-700 md:hover:bg-blue-700 px-3 py-2.5 rounded-lg font-semibold transition text-sm">
          📥 Import
        </button>
        <button onClick={handleSyncCBRE} disabled={syncing} className="flex-1 md:flex-none bg-purple-600 active:bg-purple-700 md:hover:bg-purple-700 disabled:bg-gray-600 px-3 py-2.5 rounded-lg font-semibold transition text-sm">
          {syncing ? '⏳...' : '🔄 Sync'}
        </button>
        {hasActiveFilters && (
          <button onClick={clearAllFilters} className="bg-red-700 active:bg-red-600 md:hover:bg-red-600 px-3 py-2.5 rounded-lg font-semibold transition text-sm">
            ✕ Clear
          </button>
        )}
      ...
      {hasActiveFilters && (
        <div className="mt-3 flex flex-wrap gap-2 max-h-[80px] overflow-y-auto">
          {selectedStatuses.map(status => {
            ...
            return opt ? (
              <span key={status} className={`${opt.color} text-white px-2 py-1 rounded-full text-xs flex items-center gap-1`}>
                {opt.label}
          ...
          {selectedTechs.map(techId => {
            if (techId === 'unassigned') {
              return (
                <span key={techId} className="bg-yellow-700 text-white px-2 py-1 rounded-full text-xs flex items-center gap-1">
                  ⚠️ Unassigned
            ...
            return user ? (
              <span key={techId} className="bg-blue-700 text-white px-2 py-1 rounded-full text-xs flex items-center gap-1">
          ...
          {selectedPriorities.map(priority => {
            ...
            return opt ? (
              <span key={priority} className={`${opt.color} text-white px-2 py-1 rounded-full text-xs flex items-center gap-1`}>
                {opt.label}
          ...
          {selectedCbreStatuses.map(cbreStatus => {
            ...
            return opt ? (
              <span key={cbreStatus} className={`${opt.color} text-white px-2 py-1 rounded-full text-xs flex items-center gap-1`}>
                {opt.label}
      ...
      {/* Sync Result */}
      {syncResult && (
        <div className={`mt-3 p-2 md:p-3 rounded-lg text-sm ${syncResult.success ? 'bg-green-900/50 text-green-200' : 'bg-red-900/50 text-red-200'}`}>
          {syncResult.success ? (
            <>
              ...
              {syncResult.updated > 0 && <p className="text-xs mt-2 animate-pulse">🔄 Refreshing...</p>}
            </>
          ) : (
