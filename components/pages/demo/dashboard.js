      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-amber-100 text-sm hidden sm:inline">
            - Exploring as Summit Mechanical Services. Changes are temporary.
          </span>
        </div>
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
      {cards.map((card) => (
        <button
          ...
          className={`bg-white rounded-xl p-4 shadow-sm border-2 transition-all hover:shadow-md text-left ${
            currentFilter === card.filter ? 'border-blue-500 ring-2 ring-blue-200' : 'border-transparent'
          }`}
        >
...
function WorkOrdersTable({ filter, onSelectWorkOrder }) {
  const { workOrders, getUserById } = useDemo();
    ...
    <div className="bg-white rounded-xl shadow-sm overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-200">
        <h2 className="text-lg font-semibold text-gray-800">
          Work Orders {filter && <span className="text-blue-500">({filteredOrders.length})</span>}
        </h2>
      </div>
              ...
              return (
                ...
                >
                  <td className="px-6 py-4">
                    <span className="text-blue-600 font-medium">{wo.wo_number}</span>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${getPriorityBadge(wo.priority)}`}>
                      {wo.priority}
                    </span>
                  </td>
                  <td className="px-6 py-4">
...
function WorkOrderDetailModal({ workOrder, onClose }) {
  ...
  const quotes = getQuotesForWorkOrder(workOrder.id);
  ...
  const handleAddComment = () => {
    ...
    addComment(workOrder.id, 'demo-user-002', newComment);
    setNewComment('');
  };
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex border-b border-gray-200 px-6 overflow-x-auto">
          {['details', 'team', 'comments', 'costs', 'quotes'].map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-4 py-3 font-medium text-sm capitalize transition whitespace-nowrap ${
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap gap-2">
                {workOrder.status === 'new' && (
                  <button onClick={() => setShowAssignModal(true)} className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium transition">
                    Assign Tech
                  </button>
                )}
                {workOrder.status === 'assigned' && (
                  <button onClick={() => checkIn(workOrder.id)} className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg text-sm font-medium transition">
                    Check In
                  </button>
                )}
                {workOrder.status === 'in_progress' && !workOrder.check_out_time && (
                  <button onClick={() => checkOut(workOrder.id)} className="bg-amber-500 hover:bg-amber-600 text-white px-4 py-2 rounded-lg text-sm font-medium transition">
                    Check Out
                  </button>
                )}
                {workOrder.status === 'in_progress' && (
                  <button onClick={() => completeWorkOrder(workOrder.id)} className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg text-sm font-medium transition">
                    Mark Complete
                  </button>
                )}
                {workOrder.status === 'completed' && (
                  <button onClick={() => updateStatus(workOrder.id, 'invoiced')} className="bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded-lg text-sm font-medium transition">
                    Mark Invoiced
                  </button>
                )}
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <p className="text-gray-900">{workOrder.client_name}</p>
                </div>
                <div>
                  <h4 className="text-sm font-medium text-gray-500 mb-1">Assigned Tech</h4>
                  <p className="text-gray-900">{assignedTech?.name || 'Unassigned'}</p>
                </div>
                <div>
                  <h4 className="text-sm font-medium text-gray-500 mb-1">Scheduled Date</h4>
                  <p className="text-gray-900">{workOrder.scheduled_date || 'Not scheduled'}</p>
                </div>
                <div>
                  <h4 className="text-sm font-medium text-gray-500 mb-1">NTE Amount</h4>
                  <p className="text-gray-900 font-semibold">${workOrder.nte_amount?.toLocaleString()}</p>
                </div>
                <div>
              ...
              {workOrder.check_in_time && (
                <div className="grid md:grid-cols-2 gap-6 p-4 bg-gray-50 rounded-lg">
                  <div>
                    ...
                    <p className="text-gray-900">{new Date(workOrder.check_in_time).toLocaleString()}</p>
                  </div>
                  {workOrder.check_out_time && (
                    <div>
                      <h4 className="text-sm font-medium text-gray-500 mb-1">Check Out</h4>
                      <p className="text-gray-900">{new Date(workOrder.check_out_time).toLocaleString()}</p>
                    </div>
                  )}
                </div>
              )}
          ...
          {activeTab === 'comments' && (
            <div className="space-y-4">
              <div className="flex gap-2">
                ...
                />
                <button onClick={handleAddComment} className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg font-medium transition">
                  Add
                </button>
              </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-blue-50 rounded-lg">
                  <div className="text-sm text-blue-600 mb-1">Labor</div>
                  <div className="text-2xl font-bold text-blue-700">${laborTotal.toFixed(2)}</div>
                </div>
                <div className="p-4 bg-green-50 rounded-lg">
                  <div className="text-sm text-green-600 mb-1">Materials</div>
                  <div className="text-2xl font-bold text-green-700">${materialTotal.toFixed(2)}</div>
                </div>
                <div className="p-4 bg-purple-50 rounded-lg">
                  <div className="text-sm text-purple-600 mb-1">Equipment</div>
                  <div className="text-2xl font-bold text-purple-700">${equipmentTotal.toFixed(2)}</div>
                </div>
                <div className="p-4 bg-amber-50 rounded-lg">
                  <div className="text-sm text-amber-600 mb-1">Mileage</div>
                  <div className="text-2xl font-bold text-amber-700">${mileageTotal.toFixed(2)}</div>
                </div>
              </div>
                    ...
                    {dailyHours.map((entry) => (
                      <div key={entry.id} className="p-3 bg-gray-50 rounded-lg flex items-center justify-between">
                        ...
                        <div className="text-right">
                          ...
                          {entry.mileage > 0 && <div className="text-xs text-gray-500">{entry.mileage} miles</div>}
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div className="space-y-4">
              {quotes.length > 0 ? quotes.map((quote) => (
                <div key={quote.id} className="p-4 border border-gray-200 rounded-lg">
                  ...
                  <div className="text-2xl font-bold text-gray-900 mb-2">${quote.total_cost?.toLocaleString()}</div>
                  <p className="text-sm text-gray-600 mb-3">{quote.notes}</p>
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div><span className="text-gray-500">Labor:</span> ${quote.labor_cost}</div>
                    <div><span className="text-gray-500">Materials:</span> ${quote.material_cost}</div>
                    <div><span className="text-gray-500">Equipment:</span> ${quote.equipment_cost || 0}</div>
                    <div><span className="text-gray-500">Mileage:</span> ${quote.mileage_cost}</div>
                  </div>
                </div>
              )) : (
                <p className="text-gray-500 text-center py-8">No NTE requests</p>
              )}
            </div>
          )}
        </div>
        ...
        {showAssignModal && (
          <div className="absolute inset-0 bg-black/50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl p-6 w-full max-w-md">
              ...
              <div className="space-y-3">
                {leadTechs.map((tech) => (
                  <button
                    ...
                    className="w-full p-3 text-left border border-gray-200 rounded-lg hover:bg-blue-50 hover:border-blue-500 transition"
                  >
              ...
              <button onClick={() => setShowAssignModal(false)} className="mt-4 w-full py-2 text-gray-600 hover:bg-gray-100 rounded-lg transition">
                Cancel
              </button>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-bold text-gray-800">PCS FieldService</h1>
                <p className="text-sm text-gray-500">Summit Mechanical Services</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <Link href="/demo/mobile" className="text-blue-600 hover:text-blue-700 text-sm font-medium">
                Try Mobile App →
              </Link>
              <Link href="/demo" className="text-gray-500 hover:text-gray-700 text-sm">
                Back to Demo
              </Link>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-6 text-white">
            ...
            <div className="text-blue-100">Total Work Orders</div>
          </div>
          <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-6 text-white">
            <div className="text-3xl font-bold">{stats.techsOnField}</div>
            <div className="text-green-100">Techs on Field</div>
          </div>
          <div className="bg-gradient-to-br from-amber-500 to-amber-600 rounded-xl p-6 text-white">
            <div className="text-3xl font-bold">{stats.scheduledToday}</div>
            <div className="text-amber-100">Scheduled Today</div>
          </div>
          <div className="bg-gradient-to-br from-red-500 to-red-600 rounded-xl p-6 text-white">
            <div className="text-3xl font-bold">{stats.emergency}</div>
            <div className="text-red-100">Emergencies</div>
          </div>
        </div>
        ...
        {filter && (
          <div className="mb-4">
            <button onClick={() => setFilter(null)} className="text-sm text-blue-600 hover:text-blue-700">
              ← Clear filter
            </button>
          </div>
        )}
...
export default function DemoDashboard() {
  return (
    <>
      <Head>
        <title>Dashboard Demo - PCS FieldService</title>
        <meta name="description" content="Interactive demo of PCS FieldService dashboard" />
