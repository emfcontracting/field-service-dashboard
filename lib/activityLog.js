// lib/activityLog.js
import { parseDate, tzDate } from './dates';
// ─────────────────────────────────────────────────────────────────────────────
...
// "[9/3/2026, 10:11:11 AM]" is the technician's Eastern wall clock (written with
// toLocaleString on the device). Parse the parts and pin them to Eastern time so
// the timeline is right on a server in UTC and on a browser outside ET.
export function parseCommentStamp(text) {
  const m = String(text || '').match(/(\d{1,2})\/(\d{1,2})\/(\d{4}),?\s+(\d{1,2}):(\d{2})(?::(\d{2}))?\s*([AP]M)?/i);
  if (!m) { const d = new Date(text); return isNaN(d.getTime()) ? null : d; }
  let h = +m[4];
  if (m[7]) { h = h % 12; if (m[7].toUpperCase() === 'PM') h += 12; }
  const d = tzDate(+m[3], +m[1], +m[2], h, +m[5]);
  if (m[6]) d.setSeconds(+m[6]);
  return d;
}
...
function parseCheckIns(comments) {
  ...
  while ((m = CHECKIN_REGEX.exec(comments)) !== null) {
    ...
    const ts = parseCommentStamp(timestamp);
    if (!ts) continue;
...
const safeDate = (d) => parseDate(d);   // DATE columns → midnight ET, naive stamps → UTC (lib/dates)
...
export async function buildActivityLog(supabase, woId) {
  ...
  if (safeDate(wo.date_entered)) events.push({
    ts: parseDate(wo.date_entered), category: 'lifecycle',
  ...
  if (safeDate(wo.assigned_to_field_at)) events.push({
    ts: parseDate(wo.assigned_to_field_at), category: 'lifecycle',
  ...
  if (safeDate(wo.date_completed)) events.push({
    ts: parseDate(wo.date_completed), category: 'lifecycle',
