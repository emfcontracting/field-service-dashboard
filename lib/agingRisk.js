...
import { extractPriorityCode } from './priorityCodes';
...
// Roughly what CBRE's target completion window is per priority, in calendar
// days, used only when the work order carries no target of its own. Business
// days are converted generously — the point is a basis, not a promise.
const COMPLETION_WINDOW_DAYS = {
  P1: 1, P2: 1, P3: 5, P4: 10, P5: 20, P6: 30,
  P7: 30,                      // SLA extended by CBRE; the close-out timer still runs
  P10: 30, P11: 30, P23: 3,
};
...
export function agingBasis(wo) {
  ...
  const days = code ? COMPLETION_WINDOW_DAYS[code] : null;
  if (days == null) return { date: entered, source: 'dispatch' };
  return { date: new Date(entered.getTime() + days * MS_D), source: 'estimated' };
}
