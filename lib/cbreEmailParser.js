...
// CBRE work-order number shapes. Prefixes seen so far: C (corrective), P
// (PM), PJ (project), ST (storm), COU, and since 2026-09-08 B ("billing"
// sub work orders CBRE issues for completed work whose original aged out).
// Three producers and the sub-WO linker used to carry their own copy of this
// list and all silently skipped the B-numbers.
export const CBRE_WO_PATTERN = /^(B|C|P|PJ|ST|COU)\d+$/i;
export const WO_REF_PATTERN = /\b(?:B|C|P|PJ|ST|COU)\d{7}\b/gi;
export const isCbreWoNumber = (n) => CBRE_WO_PATTERN.test(String(n || '').trim());
...
export function classifySubject(subject) {
  ...
  // "P1-Emergency Request # C3067674 Will Be Closed in 7 Days" — CBRE's
  // automated close-out warning. Seven days' notice before the work order can
  // no longer be billed; parsed in lib/cbreCloseoutNotice.js.
  if (/will\s+be\s+closed\s+in\s+\d+\s+days?/.test(s)) return 'closeout_warning';
