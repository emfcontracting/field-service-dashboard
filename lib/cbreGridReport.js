...
export function reconcileGrid(orders, woByNumber, fsmOpen = []) {
  ...
  const rows = orders.map((o) => {
    ...
    if (wo.is_locked || hasInvoice) {
      return { ...o, wo, outcome: 'billed_ours',
               detail: (wo.invoices || [])[0]?.invoice_number || 'locked here' };
    }
    if (wo.acknowledged || ['open', 'escalated', 'sub_wo_requested', 'superseded'].includes(wo.dispute_status)) {
      return { ...o, wo, outcome: 'closed_ours',
               detail: wo.dispute_status ? `escalated (${wo.dispute_status})` : 'acknowledged here' };
    }

    // The one code that carries a cbre_status fact.
    if (o.grid_code === 'QUA') {
      const cs = wo.cbre_status;
      if (cs === 'quote_approved') {
        return { ...o, wo, outcome: 'open_both', detail: 'quote approved on both sides' };
      }
      if (BEYOND_QUOTE.includes(cs) || BEYOND_QUOTE.includes(wo.cbre_posting_status)) {
        return { ...o, wo, outcome: 'open_both',
                 detail: `FSM is further along (${cs || wo.cbre_posting_status}) — left alone` };
      }
      if (BEFORE_APPROVAL.includes(cs)) {
        return { ...o, wo, outcome: 'nte_approved',
                 detail: `${cs ? String(cs).replace(/_/g, ' ') : 'nothing recorded'} → quote approved` };
      }
      return { ...o, wo, outcome: 'status_conflict',
               detail: `FSM says ${String(cs).replace(/_/g, ' ')}, CBRE says quote approved and still open` };
    }

    return { ...o, wo, outcome: 'open_both', detail: wo.status };
  });
