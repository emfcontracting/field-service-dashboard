...
//   Approval workflow: CIS → CA1 → CA2 → CIR → CMP (approvers before CIR)
// ─────────────────────────────────────────────────────────────────────────────
...
export const DEFAULT_CBRE_MAPPING = {
  ...
  'CIR': {
    ...
    set_cmp_date: true,                         // 75-day payout clock starts at CIR or CMP, whichever shows first (Daniel, Sep 2026)
    notes: 'Invoice approved by CBRE (after CA1/CA2). The 75-day payout clock starts here (or at CMP).',
  },
  'CA1': {
  ...
  'CMP': {
    ...
    target_type: 'invoice',
    target_status: 'accepted',
    set_cmp_date: true,                         // ← sets cmp_date if not yet set (CIR may have set it earlier)
    notes: 'Posted by CBRE. Payment expected 75 days from when CIR/CMP first showed.',
  },

  'EBO': {
    label: 'Equipment on back order',
    target_type: 'ignore',
    target_status: '',
    set_cmp_date: false,
    notes: 'Stop-the-clock: sync opens/closes an equipment_backorder pause — waiting on parts must not count against EMF KPIs.',
  },
  ...
  'CIA': {
    ...
    set_cmp_date: false,
    notes: 'Invoice was submitted before the PMI checklist reached the Smartsheet. Put the checklist in, then e-mail UPSPMISupport@cbre.com so the invoice is routed back for approval — nothing moves on its own.',
  },
  'CIC': {
