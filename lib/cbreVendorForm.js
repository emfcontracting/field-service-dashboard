...
import { CBRE_TZ, tzParts, tzDate, parseDate } from './dates';
...
export const ACTIONS = {
  ...
  // Same Action string as cbre_eta — CBRE's form has no separate "Site Arrival"
  // option, and its eleven Actions are fixed. This kind exists so the automatic
  // arrival reporter can be idempotent per work order and labelled honestly in
  // the Approvals tab, without colliding with the manual ETA button.
  cbre_arrival:       { value: 'Update Next Arrival Time',         label: 'Report Site Arrival',needs: ['arrival'] },
...
// ── time zone ────────────────────────────────────────────────────────────────
// Everything CBRE sees is Eastern time. Parsing/formatting lives in lib/dates.js
// (naive UTC timestamps, DATE columns, Vercel running in UTC — see there);
// re-exported here because the form code and its callers import from this file.
export { CBRE_TZ, tzParts, tzDate };
...
export function fmtDate(d, tz = CBRE_TZ) {
  const p = tzParts(parseDate(d), tz);
  if (!p) return null;
...
// Display only ("2,335.00"). NEVER put this in the payload — see moneyPlain.
...
// The value the form's NTE field accepts: a plain number, NO thousands
// separator ("2335.00"). CBRE's "NTE Request Amount" is a numeric input and
// silently refuses "2,335.00" — the field showed the amount but the form would
// not take it.
...
export function buildCbrePayload(input) {
  ...
  if (input.kind === 'cbre_nte') {
    const amt = moneyPlain(input.nteAmount);
    if (!amt) problems.push('missing / invalid NTE amount');
    // payload = plain number for the form field; readable = pretty, for our own
    // titles and the approval list.
    else { payload[K.nteAmount] = amt; readable.nteAmount = money(input.nteAmount); }
  }
  if (input.kind === 'cbre_target_date') {
    applyDT(input.targetAt, K.targetDate, K.targetTime, K.targetAmpm, 'Target');
  }
  // Driven by `needs` rather than the kind, so cbre_eta and cbre_arrival — two
  // kinds, one Action — both fill the arrival fields.
  if (a.needs.includes('arrival')) {
...
//
// NOTE: more than one kind can share an Action string (cbre_eta and
// cbre_arrival are both "Update Next Arrival Time"), so this returns only the
// first. Anything matching a confirmation back to a queued row must use
// kindsFromActionValue below, or it will miss rows of the second kind.
export function kindFromActionValue(value) {
  const all = kindsFromActionValue(value);
  return all.length ? all[0] : null;
}

// Every kind that submits this Action string. Order follows ACTIONS.
export function kindsFromActionValue(value) {
  if (!value) return [];
  const v = String(value).trim().toLowerCase();
  return Object.entries(ACTIONS)
    .filter(([, def]) => def.value.toLowerCase() === v)
    .map(([kind]) => kind);
}
