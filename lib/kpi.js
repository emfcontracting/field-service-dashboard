...
import { isSlaExtended } from './priorityCodes';

const MS_H = 3600000;
const MS_D = 86400000;

// DATE columns → midnight Eastern, tz-less stamps → UTC (lib/dates). Before
// this, date_entered parsed as UTC midnight (8 PM ET the evening before) and
// time_in as local time, which padded every response time by 4–9 hours.
export const toDate = (v) => parseDate(v);
...
// Overlapping pauses (a technician's "waiting for parts" while the quote sits
// with CBRE's approver — one open pause per WO+reason is allowed) are merged
// first, so a minute of paused time is only ever subtracted once (M12).
export function pauseMsInWindow(pauses, from, to) {
  ...
  let ms = 0, curLo = null, curHi = null;
  for (const [lo, hi] of spans) {
    if (curHi != null && lo <= curHi) { curHi = Math.max(curHi, hi); continue; }
    if (curHi != null) ms += curHi - curLo;
    curLo = lo; curHi = hi;
  }
  if (curHi != null) ms += curHi - curLo;
  return ms;
...
// can't be judged (no target, no completion date, or CBRE lifted the deadline).
export function onTimeCompletion(wo, pauses, { adjusted = true, now = new Date() } = {}) {
  // P7: CBRE itself extended the SLA on this work order, so there is no
  // deadline left to be late against. Scoring it either way would be an
  // invention — unjudgeable, exactly like a missing target.
  if (isSlaExtended(wo?.priority)) return null;
...
// Re-exported from lib/clockPause.js, which is now the single place the pause
// reasons are defined — the badge in the work orders table, the panel in the
// detail modal and this KPI view must not drift apart.
export { PAUSE_REASON_LABELS } from './clockPause';
