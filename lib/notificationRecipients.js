...
export async function sendSubscribedNotification(supabase, payload) {
  const { type, workOrder, quote, actorName, missingDataItems, updateRequiredItems, excludeUserId } = payload;
  ...
  try {
    // Works from the browser (signed-in user token) AND from API routes on the
    // server (absolute URL + CRON_SECRET) — a relative fetch has no origin in Node.
    ...
    const base = isServer
      ...
      : '';
    const doFetch = isServer ? fetch : apiFetch;
    const response = await doFetch(`${base}/api/notifications`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        ...(isServer && process.env.CRON_SECRET ? { Authorization: `Bearer ${process.env.CRON_SECRET}` } : {}),
      },
      body: JSON.stringify({
        type,
        recipients,
