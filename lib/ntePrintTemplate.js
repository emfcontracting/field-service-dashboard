...
export function buildNtePrintHTML({ workOrder, quote, materials = [] }) {
  ...
  // Show every filled description field (Notes is intentionally excluded).
  const scopeBlock = (label, val) =>
    val && String(val).trim()
      ? `<div class="scope-block"><div class="scope-label">${label}</div><div class="scope-text">${nl2br(val)}</div></div>`
      : '';
  let scopeHTML =
    scopeBlock('Troubleshooting Findings — What Was Found On Site', q.troubleshooting_findings) +
    scopeBlock('Work Required / Next Steps', q.work_required) +
    scopeBlock('Parts &amp; Materials Needed', q.parts_materials) +
    scopeBlock('Description of Additional Work', q.description);
  if (!scopeHTML) {
