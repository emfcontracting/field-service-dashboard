...
/**
 * Subcontractor portal (/contractor): a signed-in technician who has an
 * ENABLED subcontractor_profiles row — or office/admin (who review their
 * invoices). Contractors sign in through /api/contractor/login, which issues
 * the same app token the technician app uses, so getPrincipal resolves them
 * as kind 'app'. On success `principal.contractor` carries the profile row.
 */
export async function requireContractorOrStaff(request) {
  const p = await getPrincipal(request);
  if (!p || !p.user) return deny(401, 'Sign in required');
  if (STAFF_ROLES.includes(p.user.role)) return { ok: true, principal: p };
  const { data: profile } = await serviceClient()
    .from('subcontractor_profiles')
    .select('*')
    .eq('user_id', p.user.user_id)
    .maybeSingle();
  if (!profile || profile.is_enabled === false) return deny(403, 'Subcontractor access required');
  if (profile.subscription_status === 'expired') return deny(403, 'Subcontractor subscription expired');
  return { ok: true, principal: { ...p, contractor: profile } };
}

/**
 * A contractor may only touch their own rows; staff may touch anyone's.
 * `auth` is the result of requireContractorOrStaff.
 */
export function ownsUser(auth, userId) {
  const p = auth?.principal;
  if (!p?.user) return false;
  if (STAFF_ROLES.includes(p.user.role)) return true;
  return !!userId && String(userId) === String(p.user.user_id);
}
export function denyNotOwner() {
  return deny(403, 'You can only access your own records');
}
